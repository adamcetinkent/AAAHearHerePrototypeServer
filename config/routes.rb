Rails.application.routes.draw do
  ...
  get     'posts/by/:by_user_id/for/:for_user_id(/before/:date)(/exclude/:exclude)',
                            :to => 'posts#by_user', 
                            :constraints => {:date => /\d{4}-\d{2}-\d{2}%20\d{2}:\d{2}:\d{2}[.][0-9]+/,
                                             :excludeIDs => /(\d+,)+\d+|\d+/}
  get     'posts/for/:user_id(/before/:date)(/exclude/:exclude)',  
  ...
  get     'posts/for/:user_id/since/:date',  
                            ...
                            :constraints => {:date => /\d{4}-\d{2}-\d{2}%20\d{2}:\d{2}:\d{2}[.][0-9]+/}
  get     'posts/for/:user_id/at/:lat/:lon', 
                            :to => 'posts#for_user_at_location', 
                            :constraints => {:lat => /\-?\d+(.\d+)?/, :lon => /\-?\d+(.\d+)?/}
  get     'posts/for/:user_id/within/:latSW/:latNE/:lonSW/:lonNE(/exclude/:excludeIDs)', 
